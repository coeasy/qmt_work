...
# ---------- 多维行情子路由（P1-1 拆分，实现见 app/routes/market_multidim.py） ----------
# 在原位置挂载：include_router 按调用顺序追加子路由，
# 因此对外端点顺序与拆分前逐条一致。
router.include_router(market_multidim.router)
...
async def dataset_snapshots(dataset_id: str = "cn_equity_daily", limit: int = 20, ctx: AppContext = Depends(get_ctx)):
    ...
    rows = DatasetSnapshotStore(ctx.db).list_recent(dataset_id, limit)
