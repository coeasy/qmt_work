...
- 版本：`0.3.0`（单一真源为仓库根 `VERSION`，构建时随包分发并由 `build_exe.py` 校验）
- 许可证：Apache 2.0
...
> 截图取自**打包态桌面客户端** `qmt_work.exe`（v0.3.0），通过 CDP 接管真实窗口截取，非 mock、非设计稿。
...
**选股工作台** —— 条件树 JSON + 股票池 + 数据源策略

![选股工作台](docs/screenshots/03-screen.png)

**模拟盘** —— 虚拟撮合账本（100 万初始资金）；行情冻结时市值退化为成本价估算并显式标注

![模拟盘](docs/screenshots/05-paper.png)

**系统状态** —— 行情通道 / 活跃订阅 / REST 能力数 / 数据源与降级链 / 健康检查（版本号取自包内 `VERSION`，由构建期强制校验）

![系统状态](docs/screenshots/06-sysstatus.png)
...
**查看完整工具清单**：`GET /api/v1/capabilities/mcp` 返回全部工具名、按前缀分组计数与接入信息；桌面客户端「系统 → MCP 工具」页可直接浏览、按名称/分组过滤并一键复制。契约基线固化在 `backend/tests/contracts/mcp_tools.json`（语义：基线 ⊆ 现网，删除/改名即红灯）。
...
| 层级 | 命令 | 覆盖范围 |
|------|------|----------|
| 后端单测 | `cd backend && for f in tests/test_*.py; do python -m pytest "$f" -q -p no:cacheprovider; done` | 150 个 `test_*.py`，1606 个用例 |
...
│  ├─ tests/            # 150 个 test_*.py（1606 用例）+ 冒烟测试 smoke2.py
