...
| 层级 | 命令 | 覆盖范围 |
|------|------|----------|
| 后端单测 | `cd backend && for f in tests/test_*.py; do python -m pytest "$f" -q -p no:cacheprovider; done` | 150 个 `test_*.py`，1608 个用例 |
...
│  ├─ tests/            # 150 个 test_*.py（1608 用例）+ 冒烟测试 smoke2.py
