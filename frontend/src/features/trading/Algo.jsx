...
export default function Algo() {
  ...
  // C-P4：算法单为真实交易动作，未连接券商时禁用提交与暂停/恢复/取消
  ...
  useServerEvents([WS_EVENTS.ALGO], () => { load(); });
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="row">
          ...
          <button onClick={requestSubmit} disabled={!tradingReady}>提交算法单</button>
          {!tradingReady && <span className="tag warn">交易未就绪：未连接券商，无法提交算法单</span>}
        </div>
      </div>
            ...
            <tbody>
              {jobs.slice().reverse().map((j) => (
                <tr key={j.algo_id}>
                  ...
                  <td>
                    {j.status === "running" && <button onClick={() => actWithConfirm(j.algo_id, api.algoPause, "暂停算法单")} disabled={!tradingReady}>暂停</button>}
                    {j.status === "paused" && <button onClick={() => actWithConfirm(j.algo_id, api.algoResume, "恢复算法单")} disabled={!tradingReady}>恢复</button>}
                    {["pending", "running", "paused"].includes(j.status) &&
                      <button onClick={() => actWithConfirm(j.algo_id, api.algoCancel, "取消算法单")} disabled={!tradingReady}>取消</button>}
                  </td>
                </tr>
              ))}
            </tbody>
