...
export default function Trade({ params } = {}) {
  ...
  // V9 §17 Capability Gate：交易动作依赖 Execution Capability，
  // 未连接券商时禁用提交/撤单类按钮（查询与研究不受影响）。
  const tradingReady = can("trading") && !!activeBroker;
  ...
  useServerEvents([WS_EVENTS.ORDER, WS_EVENTS.DEAL, WS_EVENTS.TRADE, WS_EVENTS.CONDITION, "signal", WS_EVENTS.ALGO, "reconcile", "risk."], () => { load(); });
    ...
    <div>
      ...
      {tab === "order" && (
        <div className="card">
          ...
          <div className="row">
            ...
            <button onClick={submitOrder} disabled={!tradingReady || busy}
                    title={tradingReady ? "" : "未连接券商：请到「券商连接」页添加并连接"}>
              提交订单</button>
            <button onClick={runPrecheck} disabled={precheck?.loading}>风控预检</button>
            {!tradingReady && <span className="tag warn">交易未就绪：未连接券商，仅可查询与预检</span>}
            ...
            <tbody>
              {orders.map((o, i) => (
                <tr key={i}><td className="code">{o.order_id}</td><td className="code">{o.code}</td>
                  ...
                  <td><button onClick={() => cancelOrder(o.order_id)} disabled={!tradingReady}
                              title={tradingReady ? "" : "未连接券商，无法撤单"}>撤单</button></td></tr>
              ))}
      ...
      {tab === "condition" && (
        <div className="grid grid-2">
          <div className="card">
            ...
            <div className="btn-row"><button onClick={submitCond} disabled={!tradingReady}
                    title={tradingReady ? "" : "未连接券商：条件单触发后需真实下单，请先连接券商"}>提交条件单</button></div>
          </div>
          <div className="card">
            <h3>条件单列表</h3>
            <table>
      ...
      {tab === "target" && (
        <div className="card">
          ...
          <div className="btn-row"><button onClick={submitTarget} disabled={!tradingReady}
                  title={tradingReady ? "" : "未连接券商：实际下单不可用，仅可生成计划"}>生成调仓计划</button></div>
