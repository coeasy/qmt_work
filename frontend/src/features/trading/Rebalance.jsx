...
export default function Rebalance() {
  ...
  // C-P4：勾选「真实下单」时本次提交会真实交易，需 can("trading") 且已连接券商；
  // 仅生成计划（doTrade=false）时不受此限。
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="btn-row">
          ...
          <button onClick={submit} disabled={busy || (doTrade && !tradingReady)}>{busy ? "生成中…" : (doTrade ? "生成并下单" : "生成调仓单")}</button>
        </div>
        {doTrade && !tradingReady && <div className="otm-pre err">交易未就绪：未连接券商或无交易权限，无法真实下单（可取消「真实下单」仅生成计划）</div>}
      </div>
