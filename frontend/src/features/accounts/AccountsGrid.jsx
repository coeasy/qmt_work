...
export default function AccountsGrid() {
  ...
  useServerEvents([WS_EVENTS.ACCOUNT], () => { if (auto) load(); });
...
function BatchOrder({ connOptions }) {
  ...
  // C-P4：批量下单为真实交易动作，未连接券商时禁用（C3 要求 can("trading") && activeBroker）
  ...
  return (
    <div className="card">
      ...
      <div className="btn-row">
        <button onClick={submit} disabled={busy || !tradingReady}>{busy ? "提交中…" : "提交批量下单"}</button>
        {!tradingReady && <span className="tag warn">交易未就绪：未连接券商，无法批量下单</span>}
      </div>
...
function BatchCancel({ connOptions }) {
  ...
  return (
    <div className="card">
      ...
      <div className="btn-row">
        <button onClick={submit} disabled={busy || !tradingReady}>{busy ? "提交中…" : "提交批量撤单"}</button>
        {!tradingReady && <span className="tag warn">交易未就绪：未连接券商，无法批量撤单</span>}
      </div>
...
function BatchReconnect({ connOptions }) {
  ...
  // 重连为券商连接管理动作，未连接任何券商时无意义
  const reconnectReady = Boolean(activeId) || (connOptions && connOptions.length > 0);
  ...
  return (
    <div className="card">
      ...
      <div className="btn-row">
        <button onClick={submit} disabled={busy || !reconnectReady}>{busy ? "重连中…" : "提交批量重连"}</button>
        {!reconnectReady && <span className="tag warn">无可用券商连接</span>}
      </div>
