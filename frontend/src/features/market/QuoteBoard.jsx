...
import OrderTicketModal from "../trading/OrderTicketModal.jsx";
...
// 与 Watchlist.jsx / Moneyflow.jsx 共用同一把 key（v2）。
// 曾长期停留在 v1：自选股页写入 v2，报价牌却读 v1，
// 导致「自选股」视图永远只显示默认池、增删自选后报价牌不跟随（用户可见的不一致）。
...
function derive(q) {
  ...
  // 注意：判定基准价必须用 pre != null 而非 truthy pre——
  // 用 `pre ?` 会把基准价恰好为 0（新股/异常快照）的行整体判为「无数据」。
  const hasPre = pre != null && pre !== 0;
  const chg = last != null && hasPre ? last - pre : null;
  const pct = chg != null && hasPre ? (chg / pre) * 100 : null;
  const amp = high != null && low != null && hasPre ? ((high - low) / pre) * 100 : null;
  return {
...
export default function QuoteBoard() {
  ...
  // 自选股变更广播：自选股页增删后广播事件，此处重读 localStorage 保持一致。
  // （v2 起自选股页是独立页面，不再有底部 Dock，但事件契约保留。）
  useEffect(() => {
    ...
    // 跨标签页 / 同标签页其他组件写入 localStorage 时同步（同页写入不触发 storage，故保留事件）
    window.addEventListener("storage", reload);
    return () => { offEvent("qmt:watch:update", reload); window.removeEventListener("storage", reload); };
  }, [board]);
          ...
          {rankGroups.map((g) => (
            <div className="qb-rank-col" key={g.k}>
              ...
              <table className="qb-table qb-rank-table">
                <thead>
                  <tr><th className="num">#</th><th>代码</th><th>名称</th><th className="num">最新</th><th className="num">{g.k === "pct" ? "涨幅" : g.k === "amp" ? "振幅" : "成交额"}</th><th style={{ width: 84 }}>操作</th></tr>
                </thead>
                <tbody>
                  {g.rows.length === 0 ? (
                    <tr><td colSpan={6} className="muted" style={{ textAlign: "center", padding: 8 }}>无数据</td></tr>
                  ) : g.rows.map((r, i) => (
          ...
          <tbody>
            {sorted.length === 0 ? (
              <tr><td colSpan={COLS.length + 1} className="muted" style={{ textAlign: "center", padding: 20 }}>
                {board === "watch" ? "自选股为空，可在顶部菜单「交易 › 自选股」页添加" : "无数据"}
              </td></tr>
            ) : sorted.map((r) => (
              <tr key={r.code} onClick={() => openStock(r.code)}>
                {COLS.map((c) => (
